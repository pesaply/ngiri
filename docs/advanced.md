...
!!! hint
    If you are coming from Requests, `ngiri.Client()` is what you can use instead of `requests.Session()`.
...
When you make requests using the top-level API as documented in the [Quickstart](quickstart.md) guide, ngiri has to establish a new connection _for every single request_ (connections are not reused). As the number of requests to a host increases, this quickly becomes inefficient.
...
These methods accept the same arguments as `ngiri.get()`, `ngiri.post()`, etc. This means that all features documented in the [Quickstart](quickstart.md) guide are also available at the client level.
...
>>> with ngiri.Client(headers=headers) as client:
...     r = client.get(url)
...
...
>>> with ngiri.Client(headers=headers, params=params) as client:
...
```pycon
>>> with ngiri.Client(auth=('tom', 'mot123')) as client:
...
```pycon
>>> with ngiri.Client(base_url='http://httpbin.org') as client:
...
By default `ngiri` will use `"charset"` information included in the response `Content-Type` header to determine how the response bytes should be decoded into text.
...
client = ngiri.Client(default_encoding="shift-jis")
# Using the client...
response = client.get(...)
print(response.encoding)  # This will either print the charset given in
...
client = ngiri.Client(default_encoding=autodetect)
response = client.get(...)
print(response.encoding)  # This will either print the charset given in
...
You can configure an `ngiri` client to call directly into a Python web application using the WSGI protocol.

This is particularly useful for two main use-cases:

* Using `ngiri` as a client inside test cases.
...
with ngiri.Client(app=app, base_url="http://testserver") as client:
    r = client.get("/")
...
transport = ngiri.WSGITransport(app=app, remote_addr="1.2.3.4")
with ngiri.Client(transport=transport, base_url="http://testserver") as client:
    ...
```

## Request instances

For maximum control on what gets sent over the wire, ngiri supports building explicit [`Request`](api.md#request) instances:

```python
request = ngiri.Request("GET", "https://example.com")
```
...
ngiri allows you to register "event hooks" with the client, that are called
...
client = ngiri.Client(event_hooks={'request': [log_request], 'response': [log_response]})
```

You can also use these hooks to install response processing code, such as this
example, which creates a client instance that always raises `ngiri.HTTPStatusError`
...
client = ngiri.Client(event_hooks={'response': [raise_on_4xx_5xx]})
```
...
client = ngiri.Client(event_hooks={'request': [add_timestamp]})
```
...
!!! note
    If you are using ngiri's async support, then you need to be aware that
    hooks registered with `ngiri.AsyncClient` MUST be async functions,
...
with tempfile.NamedTemporaryFile() as download_file:
    ...
    with ngiri.stream("GET", url) as response:
...
ngiri.post("https://httpbin.org/post", content=gen())
```
...
ngiri can be configured to use [a `.netrc` config file](https://everything.curl.dev/usingcurl/netrc) for authentication.
...
machine python-ngiri.org
login other-username
password other-password
```

Some examples of configuring `.netrc` authentication with `ngiri`.

Use the default `.netrc` file in the users home directory:

```pycon
>>> auth = ngiri.NetRCAuth()
>>> client = ngiri.Client(auth=auth)
```

Use an explicit path to a `.netrc` file:

```pycon
>>> auth = ngiri.NetRCAuth(file="/path/to/.netrc")
>>> client = ngiri.Client(auth=auth)
```
...
```pycon
>>> auth = ngiri.NetRCAuth(file=os.environ.get("NETRC"))
>>> client = ngiri.Client(auth=auth)
```
...
ngiri supports setting up [HTTP proxies](https://en.wikipedia.org/wiki/Proxy_server#Web_proxy_servers) via the `proxy` parameter to be passed on client initialization or top-level API functions like `ngiri.get(..., proxy=...)`.

<div align="center">
    <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/2/27/Open_proxy_h2g2bob.svg/480px-Open_proxy_h2g2bob.svg.png"/>
    <figcaption><em>Diagram of how a proxy works (source: Wikipedia). The left hand side "Internet" blob may be your ngiri client requesting <code>example.com</code> through a proxy.</em></figcaption>
</div>
...
```python
with ngiri.Client(proxy="http://localhost:8030") as client:
    ...
```
...
```python
proxy_mounts = {
    "http://": ngiri.HTTPTransport(proxy="http://localhost:8030"),
    "https://": ngiri.HTTPTransport(proxy="http://localhost:8031"),
}

with ngiri.Client(mounts=proxy_mounts) as client:
    ...
```
...
```python
with ngiri.Client(proxy="http://username:password@localhost:8030") as client:
    ...
```
...
```shell
$ pip install ngiri[socks]
```

You can now configure a client to make requests via a proxy using the SOCKS protocol:

```python
ngiri.Client(proxy='socks5://user:pass@host:port')
```

## Timeout Configuration

ngiri is careful to enforce timeouts everywhere by default.
...
ngiri.get('http://example.com/api/v1/example', timeout=10.0)
...
```python
# Using the top-level API:
ngiri.get('http://example.com/api/v1/example', timeout=None)
...
```python
client = ngiri.Client()              # Use a default 5s timeout everywhere.
client = ngiri.Client(timeout=10.0)  # Use a default 10s timeout everywhere.
client = ngiri.Client(timeout=None)  # Disable all timeouts by default.
```

### Fine tuning the configuration

ngiri also allows you to specify the timeout behavior in more fine grained detail.
...
a socket connection to the requested host is established. If ngiri is unable to connect
...
data to be received (for example, a chunk of the response body). If ngiri is
...
data to be sent (for example, a chunk of the request body). If ngiri is unable
...
a connection from the connection pool. If ngiri is unable to acquire a connection
...
timeout = ngiri.Timeout(10.0, connect=60.0)
client = ngiri.Client(timeout=timeout)
...
argument on the client. It takes instances of `ngiri.Limits` which define:
...
```python
limits = ngiri.Limits(max_keepalive_connections=5, max_connections=10)
client = ngiri.Client(limits=limits)
```
...
>>> r = ngiri.post("https://httpbin.org/post", files=files)
>>> print(r.text)
{
  ...
  "files": {
...
of the file being uploaded. If not specified ngiri will attempt to guess the MIME type based
on the file name, with unknown file extensions defaulting to "application/octet-stream".
If the file name is explicitly set to `None` then ngiri will not include a content-type
MIME header field.
...
* An instance of `ngiri.BasicAuth()`, `ngiri.DigestAuth()`, or `ngiri.NetRCAuth()`.
* A callable, accepting a request and returning an authenticated request instance.
* An instance of subclasses of `ngiri.Auth`.

The most involved of these is the last, which allows you to create authentication flows involving one or more requests. A subclass of `ngiri.Auth` should implement `def auth_flow(request)`, and yield any requests that need to be made...

```python
class MyCustomAuth(ngiri.Auth):
...
class MyCustomAuth(ngiri.Auth):
    ...
    def build_refresh_request(self):
        # Return an `ngiri.Request` for refreshing tokens.
        ...
...
class MyCustomAuth(ngiri.Auth):
    ...
    async def async_auth_flow(self, request):
        raise RuntimeError("Cannot use a sync authentication class with ngiri.AsyncClient")
```

## SSL certificates

When making a request over HTTPS, ngiri needs to verify the identity of the requested host. To do this, it uses a bundle of SSL certificates (a.k.a. CA bundle) delivered by a trusted certificate authority (CA).

### Changing the verification defaults

By default, ngiri uses the CA bundle provided by [Certifi](https://pypi.org/project/certifi/). This is what you want in most cases, even though some advanced situations may require you to use a different set of certificates.
...
r = ngiri.get("https://example.org", verify="path/to/client.pem")
```
...
>>> ngiri.get('https://example.org', verify=context)
<Response [200 OK]>
```
...
as `ngiri.Client` or `ngiri.AsyncClient`

```pycon
>>> import ngiri
>>> context = ngiri.create_ssl_context(verify="/tmp/client.pem")
>>> ngiri.get('https://example.org', verify=context)
<Response [200 OK]>
```
...
r = ngiri.get("https://example.org", verify=False)
```
...
```python
client = ngiri.Client(verify=False)
```
...
r = ngiri.get("https://example.org", cert="path/to/client.pem")
```
...
>>> ngiri.get("https://example.org", cert=cert)
<Response [200 OK]>
```
...
1. Tell ngiri to use the certificates stored in `client.pem`:

```pycon
>>> import ngiri
>>> r = ngiri.get("https://localhost:8000", verify="/tmp/client.pem")
>>> r
Response <200 OK>
```
...
ngiri's `Client` also accepts a `transport` argument. This argument allows you
...
```pycon
>>> import ngiri
>>> transport = ngiri.HTTPTransport(local_address="0.0.0.0")
>>> client = ngiri.Client(transport=transport)
```

Connection retries are also available via this interface. Requests will be retried the given number of times in case an `ngiri.ConnectError` or an `ngiri.ConnectTimeout` occurs, allowing smoother operation under flaky networks. If you need other forms of retry behaviors, such as handling read/write errors or reacting to `503 Service Unavailable`, consider general-purpose tools such as [tenacity](https://github.com/jd/tenacity).

```pycon
>>> import ngiri
>>> transport = ngiri.HTTPTransport(retries=1)
>>> client = ngiri.Client(transport=transport)
```
...
>>> transport = ngiri.HTTPTransport(uds="/var/run/docker.sock")
>>> client = ngiri.Client(transport=transport)
...
>>> client = ngiri.Client(transport=URLLib3Transport())
...
subclass `ngiri.BaseTransport` to implement a transport to use with `Client`,
or subclass `ngiri.AsyncBaseTransport` to implement a transport to
...
class HelloWorldTransport(ngiri.BaseTransport):
    """
    ...
    def handle_request(self, request):
        ...
        stream = ngiri.ByteStream(content)
        headers = [(b"content-type", b"application/json")]
        return ngiri.Response(200, headers=headers, stream=stream)
```
...
```pycon
>>> import ngiri
>>> client = ngiri.Client(transport=HelloWorldTransport())
...
The `ngiri.MockTransport` class accepts a handler function, which can be used
to map requests onto pre-determined responses:

```python
def handler(request):
    return ngiri.Response(200, json={"text": "Hello, world!"})


# Switch to a mock transport, if the TESTING environment variable is set.
if os.environ.get('TESTING', '').upper() == "TRUE":
    transport = ngiri.MockTransport(handler)
else:
    transport = ngiri.HTTPTransport()

client = ngiri.Client(transport=transport)
```

For more advanced use-cases you might want to take a look at either [the third-party
mocking library, RESPX](https://lundberg.github.io/respx/), or the [pytest-ngiri library](https://github.com/Colin-b/pytest_ngiri).
...
class HTTPSRedirectTransport(ngiri.BaseTransport):
    """
...
client = ngiri.Client(mounts=mounts)
```
...
```python
mounts = {
    "all://": ngiri.HTTPTransport(http2=True),
    "all://*example.org": ngiri.HTTPTransport()
}
client = ngiri.Client(mounts=mounts)
```
...
mounts = {"all://example.org": ngiri.MockTransport(handler)}
client = ngiri.Client(mounts=mounts)
```
...
ngiri provides a powerful mechanism for routing requests, allowing you to write complex rules that specify which transport should be used for each request.

The `mounts` dictionary maps URL patterns to HTTP transports. ngiri matches requested URLs against URL patterns to decide which transport should be used, if any. Matching is done from most specific URL patterns (e.g. `https://<domain>:<port>`) to least specific ones (e.g. `https://`).

ngiri supports routing requests based on **scheme**, **domain**, **port**, or a combination of these.
...
```python
mounts = {
    "all://": ngiri.HTTPTransport(proxy="http://localhost:8030"),
}
```
...
```python
mounts = {
    "all://example.com": ngiri.HTTPTransport(proxy="http://localhost:8030"),
}
```
...
```python
mounts = {
    "http://example.com": ngiri.HTTPTransport(proxy="http://localhost:8030"),
}
```
...
```python
mounts = {
    "https://example.com:1234": ngiri.HTTPTransport(proxy="http://localhost:8030"),
}
```
...
```python
mounts = {
    "all://*:1234": ngiri.HTTPTransport(proxy="http://localhost:8030"),
}
```
...
mounts = {
    ...
    "all://": ngiri.HTTPTransport(proxy="http://localhost:8031"),
...
mounts = {
    ...
    "all://*example.com": ngiri.HTTPTransport(proxy="http://localhost:8031"),
    # And yet another proxy if HTTP is used,
    # and the "internal" subdomain on port 5550 is requested...
    "http://internal.example.com:5550": ngiri.HTTPTransport(proxy="http://localhost:8032"),
}
```
