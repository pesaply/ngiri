...
ngiri offers a standard synchronous API by default, but also gives you
...
Use `async with ngiri.AsyncClient()` if you want a context-managed client...
...
When instantiating a transport instance directly, you need to use `ngiri.AsyncHTTPTransport`.

For instance:

```pycon
>>> import ngiri
>>> transport = ngiri.AsyncHTTPTransport(retries=1)
...
ngiri supports either `asyncio` or `trio` as an async environment.
...
Just as `ngiri.Client` allows you to call directly into WSGI web applications,
the `ngiri.AsyncClient` class allows you to call directly into ASGI web applications.
...
```pycon
>>> import ngiri
>>> async with ngiri.AsyncClient(app=app, base_url="http://testserver") as client:
...     r = await client.get("/")
...
# on port 123.
transport = ngiri.ASGITransport(app=app, client=("1.2.3.4", 123))
async with ngiri.AsyncClient(transport=transport, base_url="http://testserver") as client:
    ...
```
...
It is not in the scope of ngiri to trigger lifespan events of your app.
