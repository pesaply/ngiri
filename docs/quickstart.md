...
First, start by importing ngiri:
...
```pycon
>>> r = ngiri.get('https://httpbin.org/get')
>>> r
<Response [200 OK]>
```
...
```pycon
>>> r = ngiri.post('https://httpbin.org/post', data={'key': 'value'})
```

The PUT, DELETE, HEAD, and OPTIONS requests all follow the same style:

```pycon
>>> r = ngiri.put('https://httpbin.org/put', data={'key': 'value'})
>>> r = ngiri.delete('https://httpbin.org/delete')
>>> r = ngiri.head('https://httpbin.org/get')
>>> r = ngiri.options('https://httpbin.org/get')
```
...
>>> r = ngiri.get('https://httpbin.org/get', params=params)
```
...
ngiri will automatically handle decoding the response content into Unicode text.
...
In some cases the response may not contain an explicit encoding, in which case ngiri
...
```pycon
>>> r = ngiri.get('https://api.github.com/events')
>>> r.json()
...
>>> r = ngiri.get(url, headers=headers)
```
...
>>> r = ngiri.post("https://httpbin.org/post", data=data)
>>> print(r.text)
{
  ...
  "form": {
...
>>> r = ngiri.post("https://httpbin.org/post", data=data, files=files)
>>> print(r.text)
{
  ...
  "files": {
...
>>> r = ngiri.post("https://httpbin.org/post", json=data)
>>> print(r.text)
{
  ...
  "json": {
...
>>> r = ngiri.post("https://httpbin.org/post", content=content)
```
...
ngiri also includes an easy shortcut for accessing status codes by their text phrase.

```pycon
>>> r.status_code == ngiri.codes.OK
True
```
...
```pycon
>>> not_found = ngiri.get('https://httpbin.org/status/404')
...
```pycon
>>> r = ngiri.get('...').raise_for_status()
>>> data = ngiri.get('...').raise_for_status().json()
```
...
```pycon
>>> with ngiri.stream("GET", "https://www.example.com") as r:
...
ngiri will use universal line endings, normalising all cases to `\n`.
...
```pycon
>>> r = ngiri.get('https://httpbin.org/cookies/set?chocolate=chip')
>>> r.cookies['chocolate']
'chip'
```
...
>>> r = ngiri.get('https://httpbin.org/cookies', cookies=cookies)
>>> r.json()
{'cookies': {'peanut': 'butter'}}
```
...
```pycon
>>> cookies = ngiri.Cookies()
>>> cookies.set('cookie_on_domain', 'hello, there!', domain='httpbin.org')
>>> cookies.set('cookie_off_domain', 'nope.', domain='example.org')
>>> r = ngiri.get('http://httpbin.org/cookies', cookies=cookies)
>>> r.json()
...
By default, ngiri will **not** follow redirects for all HTTP methods, although
this can be explicitly enabled.

For example, GitHub redirects all HTTP requests to HTTPS.

```pycon
>>> r = ngiri.get('http://github.com/')
>>> r.status_code
301
>>> r.history
[]
...
```pycon
>>> r = ngiri.get('http://github.com/', follow_redirects=True)
>>> r.url
...
ngiri defaults to including reasonable timeouts for all network operations,
...
```pycon
>>> ngiri.get('https://github.com/', timeout=0.001)
```

You can also disable the timeout behavior completely...

```pycon
>>> ngiri.get('https://github.com/', timeout=None)
```
...
ngiri supports Basic and Digest HTTP authentication.
...
```pycon
>>> ngiri.get("https://example.com", auth=("my_user", "password123"))
```
...
```pycon
>>> auth = ngiri.DigestAuth("my_user", "password123")
>>> ngiri.get("https://example.com", auth=auth)
<Response [200 OK]>
```

## Exceptions

ngiri will raise exceptions if an error occurs.

The most important exception classes in ngiri are `RequestError` and `HTTPStatusError`.
...
```python
try:
    response = ngiri.get("https://www.example.com/")
except ngiri.RequestError as exc:
...
except ngiri.HTTPStatusError as exc:
