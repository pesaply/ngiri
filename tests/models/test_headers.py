...
def test_headers():
    h = ngiri.Headers([("a", "123"), ("a", "456"), ("b", "789")])
    assert "a" in h
    assert "A" in h
    assert "b" in h
    assert "B" in h
...
def test_copy_headers_method():
    headers = ngiri.Headers({"custom": "example"})
...
def test_copy_headers_init():
    headers = ngiri.Headers({"custom": "example"})
    headers_copy = ngiri.Headers(headers)
    assert headers == headers_copy


def test_headers_insert_retains_ordering():
    headers = ngiri.Headers({"a": "a", "b": "b", "c": "c"})
    headers["b"] = "123"
...
def test_headers_insert_removes_all_existing():
    headers = ngiri.Headers([("a", "123"), ("a", "456")])
    headers["a"] = "789"
...
def test_headers_encoding_in_repr():
    ...
    """
    headers = ngiri.Headers({b"custom": "example ☃".encode("utf-8")})
...
def test_headers_list_repr():
    ...
    """
    headers = ngiri.Headers([("custom", "example 1"), ("custom", "example 2")])
    assert (
...
def test_headers_decode_ascii():
    ...
    headers = ngiri.Headers(raw_headers)
...
def test_multiple_headers():
    ...
    """
    h = ngiri.Headers([("set-cookie", "a, b"), ("set-cookie", "c")])
    assert h.get_list("Set-Cookie") == ["a, b", "c"]

    h = ngiri.Headers([("vary", "a, b"), ("vary", "c")])
