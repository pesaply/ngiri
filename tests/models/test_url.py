...
# Tests for `ngiri.URL` instantiation and property accessors.


def test_basic_url():
    url = ngiri.URL("https://www.example.com/")
...
def test_complete_url():
    url = ngiri.URL("https://example.org:123/path/to/somewhere?abc=123#anchor")
    assert url.scheme == "https"
...
def test_url_with_empty_query():
    ...
    """
    url = ngiri.URL("https://www.example.com/path")
...
def test_url_query_encoding():
    ...
    See https://github.com/encode/ngiri/issues/2536
    and https://github.com/encode/ngiri/discussions/2460
    """
    url = ngiri.URL("https://www.example.com/?a=b c&d=e/f")
    assert url.raw_path == b"/?a=b%20c&d=e/f"

    url = ngiri.URL("https://www.example.com/", params={"a": "b c", "d": "e/f"})
    assert url.raw_path == b"/?a=b%20c&d=e%2Ff"


def test_url_params():
    url = ngiri.URL("https://example.org:123/path/to/somewhere", params={"a": "123"})
    assert str(url) == "https://example.org:123/path/to/somewhere?a=123"
    assert url.params == ngiri.QueryParams({"a": "123"})
...
def test_url_valid_host():
    url = ngiri.URL("https://example.com/")
    assert url.host == "example.com"
...
def test_url_ipv4_like_host():
    ...
    url = ngiri.URL("https://023b76x43144/")
...
def test_url_valid_port():
    url = ngiri.URL("https://example.com:123/")
    assert url.port == 123


def test_url_normalized_port():
    # If the port matches the scheme default it is normalized to None.
    url = ngiri.URL("https://example.com:443/")
    assert url.port is None


def test_url_invalid_port():
    with pytest.raises(ngiri.InvalidURL) as exc:
        ngiri.URL("https://example.com:abc/")
...
def test_url_normalized_path():
    url = ngiri.URL("https://example.com/abc/def/../ghi/./jkl")
...
def test_url_leading_dot_prefix_on_absolute_url():
    url = ngiri.URL("https://example.com/../abc")
    assert url.path == "/abc"
...
def test_param_requires_encoding():
    url = ngiri.URL("http://webservice", params={"u": "with spaces"})
    assert str(url) == "http://webservice?u=with%20spaces"


def test_param_does_not_require_encoding():
    url = ngiri.URL("http://webservice", params={"u": "with%20spaces"})
    assert str(url) == "http://webservice?u=with%20spaces"


def test_param_with_existing_escape_requires_encoding():
    url = ngiri.URL("http://webservice", params={"u": "http://example.com?q=foo%2Fa"})
...
def test_url_with_components():
    url = ngiri.URL(scheme="https", host="www.example.com", path="/")
...
def test_url_with_relative_path():
    ...
    url = ngiri.URL(path="abc")
    assert url.path == "abc"


# Tests for `ngiri.URL` python built-in operators.


def test_url_eq_str():
    """
    Ensure that `ngiri.URL` supports the equality operator.
    """
    url = ngiri.URL("https://example.org:123/path/to/somewhere?abc=123#anchor")
...
def test_url_set():
    """
    Ensure that `ngiri.URL` instances can be used in sets.
    """
    urls = (
        ngiri.URL("http://example.org:123/path/to/somewhere"),
        ngiri.URL("http://example.org:123/path/to/somewhere/else"),
    )
...
# Tests for TypeErrors when instantiating `ngiri.URL`.


def test_url_invalid_type():
    """
    Ensure that invalid types on `ngiri.URL()` raise a `TypeError`.
    """
    ...
    with pytest.raises(TypeError):
        ngiri.URL(ExternalURLClass())  # type: ignore
...
def test_url_join():
    ...
    """
    url = ngiri.URL("https://example.org:123/path/to/somewhere")
...
def test_relative_url_join():
    url = ngiri.URL("/path/to/somewhere")
...
def test_url_join_rfc3986():
    ...
    url = ngiri.URL("http://example.com/b/c/d;p?q")
...
def test_resolution_error_1833():
    """
    See https://github.com/encode/ngiri/issues/1833
    """
    url = ngiri.URL("https://example.com/?[]")
...
def test_url_copywith_authority_subcomponents():
    ...
    url = ngiri.URL("https://example.org")
...
def test_url_copywith_security():
    ...
    """
    with pytest.raises(ngiri.InvalidURL):
        ngiri.URL("https://u:p@[invalid!]//evilHost/path?t=w#tw")

    url = ngiri.URL("https://example.com/path?t=w#tw")
...
def test_url_set_param_manipulation():
    ...
    """
    url = ngiri.URL("https://example.org:123/?a=123")
...
def test_url_unescaped_idna_host():
    url = ngiri.URL("https://中国.icom.museum/")
    assert url.raw_host == b"xn--fiqs8s.icom.museum"


def test_url_escaped_idna_host():
    url = ngiri.URL("https://xn--fiqs8s.icom.museum/")
    assert url.raw_host == b"xn--fiqs8s.icom.museum"
...
def test_url_valid_ipv4():
    url = ngiri.URL("https://1.2.3.4/")
    assert url.host == "1.2.3.4"


def test_url_invalid_ipv4():
    with pytest.raises(ngiri.InvalidURL) as exc:
        ngiri.URL("https://999.999.999.999/")
...
def test_ipv6_url():
    url = ngiri.URL("http://[::ffff:192.168.0.1]:5678/")
...
def test_ipv6_url_from_raw_url(host):
    url = ngiri.URL(scheme="https", host=host, port=443, path="/")
...
def test_ipv6_url_copy_with_host(url_str, new_host):
    url = ngiri.URL(url_str).copy_with(host=new_host)
