...
def test_cookies():
    cookies = ngiri.Cookies({"name": "value"})
...
def test_cookies_with_domain():
    ...
    with pytest.raises(ngiri.CookieConflict):
        cookies["name"]
...
def test_multiple_set_cookie():
    ...
    response = ngiri.Response(200, request=request, headers=headers)

    cookies = ngiri.Cookies(jar)
...
def test_cookies_can_be_a_list_of_tuples():
    ...
    cookies = ngiri.Cookies(cookies_val)
