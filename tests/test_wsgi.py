...
def test_wsgi():
    client = ngiri.Client(app=application_factory([b"Hello, World!"]))
...
def test_wsgi_upload():
    client = ngiri.Client(app=echo_body)
...
def test_wsgi_upload_with_response_stream():
    client = ngiri.Client(app=echo_body_with_response_stream)
    response = client.post("http://www.example.org/", content=b"example")
    assert response.status_code == 200
    assert response.text == "example"
...
def test_wsgi_exc():
    client = ngiri.Client(app=raise_exc)
    with pytest.raises(ValueError):
        client.get("http://www.example.org/")


def test_wsgi_http_error():
    client = ngiri.Client(app=partial(raise_exc, exc=RuntimeError))
...
def test_wsgi_generator():
    ...
    client = ngiri.Client(app=application_factory(output))
    response = client.get("http://www.example.org/")
    assert response.status_code == 200
...
def test_logging():
    buffer = StringIO()
    transport = ngiri.WSGITransport(app=log_to_wsgi_log_buffer, wsgi_errors=buffer)
