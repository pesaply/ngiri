...
def test_basic_auth():
    auth = ngiri.BasicAuth(username="user", password="pass")
    ...
    response = ngiri.Response(content=b"Hello, world!", status_code=200)
    with pytest.raises(StopIteration):
        flow.send(response)


def test_digest_auth_with_200():
    auth = ngiri.DigestAuth(username="user", password="pass")
    request = ngiri.Request("GET", "https://www.example.com")
...
def set_cookies(request: ngiri.Request) -> ngiri.Response:
    headers = {
...
def test_digest_auth_setting_cookie_in_request():
    ...
    client = ngiri.Client(transport=ngiri.MockTransport(set_cookies))
