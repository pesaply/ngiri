...
def test_logging_redirect_chain(server, caplog):
    caplog.set_level(logging.INFO)
    with ngiri.Client(follow_redirects=True) as client:
...
def test_same_origin():
    origin1 = ngiri.URL("https://example.com")
    origin2 = ngiri.URL("HTTPS://EXAMPLE.COM:443")
    assert same_origin(origin1, origin2)


def test_not_same_origin():
    origin1 = ngiri.URL("https://example.com")
    origin2 = ngiri.URL("HTTP://EXAMPLE.COM")
    assert not same_origin(origin1, origin2)


def test_is_https_redirect():
    url = ngiri.URL("http://example.com")
    location = ngiri.URL("https://example.com")
    assert is_https_redirect(url, location)


def test_is_not_https_redirect():
    url = ngiri.URL("http://example.com")
    location = ngiri.URL("https://www.example.com")
    assert not is_https_redirect(url, location)


def test_is_not_https_redirect_if_not_default_ports():
    url = ngiri.URL("http://example.com:9999")
    location = ngiri.URL("https://example.com:1337")
    assert not is_https_redirect(url, location)
...
def test_url_matches(pattern, url, expected):
    ...
    assert pattern.matches(ngiri.URL(url)) == expected
