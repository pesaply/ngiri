...
async def test_context_managed_transport():
    class Transport(ngiri.AsyncBaseTransport):
        def __init__(self) -> None:
            self.events: typing.List[str] = []

        async def aclose(self):
            # The base implementation of ngiri.AsyncBaseTransport just
    ...
    transport = Transport()
    async with ngiri.AsyncClient(transport=transport):
        pass
...
async def test_client_closed_state_using_implicit_open():
    client = ngiri.AsyncClient(transport=ngiri.MockTransport(hello_world))
...
async def test_client_closed_state_using_with_block():
    async with ngiri.AsyncClient(transport=ngiri.MockTransport(hello_world)) as client:
        assert not client.is_closed
        await client.get("http://example.com")
...
async def test_mounted_transport():
    ...
    async with ngiri.AsyncClient(transport=transport, mounts=mounts) as client:
...
async def test_async_mock_transport():
    async def hello_world(request: ngiri.Request) -> ngiri.Response:
        return ngiri.Response(200, text="Hello, world!")

    transport = ngiri.MockTransport(hello_world)

    async with ngiri.AsyncClient(transport=transport) as client:
        response = await client.get("https://www.example.com")
        assert response.status_code == 200
        assert response.text == "Hello, world!"
...
async def test_cancellation_during_stream():
    ...
    * `SystemExit`
    ...
    See https://github.com/encode/ngiri/issues/2139
    """
    stream_was_closed = False

    def response_with_cancel_during_stream(request):
        class CancelledStream(ngiri.AsyncByteStream):
        ...
        return ngiri.Response(
            200, headers={"Content-Length": "12"}, stream=CancelledStream()
        )

    transport = ngiri.MockTransport(response_with_cancel_during_stream)
