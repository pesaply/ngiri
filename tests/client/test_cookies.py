...
def get_and_set_cookies(request: ngiri.Request) -> ngiri.Response:
    if request.url.path == "/echo_cookies":
        ...
        return ngiri.Response(200, headers={"set-cookie": "example-name=example-value"})
    else:
...
def test_set_cookie() -> None:
    ...
    client = ngiri.Client(
        cookies=cookies, transport=ngiri.MockTransport(get_and_set_cookies)
    )
...
def test_set_per_request_cookie_is_deprecated() -> None:
    ...
    client = ngiri.Client(transport=ngiri.MockTransport(get_and_set_cookies))
