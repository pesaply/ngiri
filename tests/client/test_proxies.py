...
@pytest.mark.parametrize(
    ...
    [
        ...
        (ngiri.Proxy("http://127.0.0.1"), [("all://", "http://127.0.0.1")]),
        (
            {
                "https://": ngiri.Proxy("https://127.0.0.1"),
                "all://": "http://127.0.0.1",
            },
...
def test_proxies_parameter(proxies, expected_proxies):
    with pytest.warns(DeprecationWarning):
        client = ngiri.Client(proxies=proxies)
    ...
    for proxy_key, url in expected_proxies:
        ...
        assert isinstance(proxy, ngiri.HTTPTransport)
...
def test_socks_proxy_deprecated():
    url = ngiri.URL("http://www.example.com")

    with pytest.warns(DeprecationWarning):
        client = ngiri.Client(proxies="socks5://localhost/")
    transport = client._transport_for_url(url)
    assert isinstance(transport, ngiri.HTTPTransport)
    assert isinstance(transport._pool, httpcore.SOCKSProxy)

    with pytest.warns(DeprecationWarning):
        async_client = ngiri.AsyncClient(proxies="socks5://localhost/")
    async_transport = async_client._transport_for_url(url)
    assert isinstance(async_transport, ngiri.AsyncHTTPTransport)
...
def test_socks_proxy():
    ...
    client = ngiri.Client(proxy="socks5://localhost/")
    transport = client._transport_for_url(url)
    assert isinstance(transport, ngiri.HTTPTransport)
    assert isinstance(transport._pool, httpcore.SOCKSProxy)

    async_client = ngiri.AsyncClient(proxy="socks5://localhost/")
    async_transport = async_client._transport_for_url(url)
    assert isinstance(async_transport, ngiri.AsyncHTTPTransport)
    assert isinstance(async_transport._pool, httpcore.AsyncSOCKSProxy)
...
def test_transport_for_request(url, proxies, expected):
    ...
    transport = client._transport_for_url(ngiri.URL(url))
...
async def test_async_proxy_close():
    try:
        with pytest.warns(DeprecationWarning):
            client = ngiri.AsyncClient(proxies={"https://": PROXY_URL})
...
def test_sync_proxy_close():
    try:
        with pytest.warns(DeprecationWarning):
            client = ngiri.Client(proxies={"https://": PROXY_URL})
...
def test_unsupported_proxy_scheme_deprecated():
    with pytest.warns(DeprecationWarning), pytest.raises(ValueError):
        ngiri.Client(proxies="ftp://127.0.0.1")


def test_unsupported_proxy_scheme():
    with pytest.raises(ValueError):
        ngiri.Client(proxy="ftp://127.0.0.1")
...
@pytest.mark.parametrize("client_class", [ngiri.Client, ngiri.AsyncClient])
...
def test_proxy_and_proxies_together():
    with pytest.warns(DeprecationWarning), pytest.raises(
        ...
        ngiri.Client(proxies={"all://": "http://127.0.0.1"}, proxy="http://127.0.0.1")
...
def test_proxy_with_mounts():
    proxy_transport = ngiri.HTTPTransport(proxy="http://127.0.0.1")
    client = ngiri.Client(mounts={"http://": proxy_transport})

    transport = client._transport_for_url(ngiri.URL("http://example.com"))
