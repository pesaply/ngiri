...
def app(request: ngiri.Request) -> ngiri.Response:
    if request.url.path == "/redirect":
        return ngiri.Response(303, headers={"server": "testserver", "location": "/"})
    elif request.url.path.startswith("/status/"):
        status_code = int(request.url.path[-3:])
        return ngiri.Response(status_code, headers={"server": "testserver"})

    return ngiri.Response(200, headers={"server": "testserver"})
...
def test_event_hooks():
    ...
    with ngiri.Client(
        event_hooks=event_hooks, transport=ngiri.MockTransport(app)
    ) as http:
