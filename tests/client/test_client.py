...
def test_get(server):
    url = server.url
    with ngiri.Client(http2=True) as http:
...
def test_get_invalid_url(server, url):
    with ngiri.Client() as client:
        with pytest.raises((ngiri.UnsupportedProtocol, ngiri.LocalProtocolError)):
            client.get(url)
    ...
    with ngiri.Client() as client:
        for status_code in (200, 400, 404, 500, 505):
            ...
            if 400 <= status_code < 600:
                with pytest.raises(ngiri.HTTPStatusError) as exc_info:
...
def test_base_url(server):
    base_url = server.url
    with ngiri.Client(base_url=base_url) as client:
        response = client.get("/")
    assert response.status_code == 200
...
def test_merge_absolute_url():
    client = ngiri.Client(base_url="https://www.example.com/")
...
def test_merge_relative_url_with_path():
    client = ngiri.Client(base_url="https://www.example.com/some/path")
    request = client.build_request("GET", "/testing/123")
...
def test_merge_relative_url_with_encoded_slashes():
    ...
    client = ngiri.Client(base_url="https://www.example.com/base%2Fpath")
    request = client.build_request("GET", "/testing")
    assert request.url == "https://www.example.com/base%2Fpath/testing"


def test_context_managed_transport():
    class Transport(ngiri.BaseTransport):
        def __init__(self) -> None:
            self.events: typing.List[str] = []

        def close(self):
            # The base implementation of ngiri.BaseTransport just
    ...
    transport = Transport()
    with ngiri.Client(transport=transport):
        pass
...
def test_context_managed_transport_and_mount():
    ...
    with ngiri.Client(transport=transport, mounts={"http://www.example.org": mounted}):
        pass
...
def test_client_closed_state_using_implicit_open():
    client = ngiri.Client(transport=ngiri.MockTransport(hello_world))
...
def test_client_closed_state_using_with_block():
    with ngiri.Client(transport=ngiri.MockTransport(hello_world)) as client:
        assert not client.is_closed
        client.get("http://example.com")
...
def echo_raw_headers(request: ngiri.Request) -> ngiri.Response:
    data = [
...
def test_raw_client_header():
    ...
    client = ngiri.Client(
        transport=ngiri.MockTransport(echo_raw_headers), headers=headers
    )
    ...
    assert response.json() == [
        ...
        ["User-Agent", f"python-ngiri/{ngiri.__version__}"],
        ["Example-Header", "example-value"],
    ]


def unmounted(request: ngiri.Request) -> ngiri.Response:
    data = {"app": "unmounted"}
    return ngiri.Response(200, json=data)


def mounted(request: ngiri.Request) -> ngiri.Response:
    data = {"app": "mounted"}
    return ngiri.Response(200, json=data)


def test_mounted_transport():
    transport = ngiri.MockTransport(unmounted)
    mounts = {"custom://": ngiri.MockTransport(mounted)}

    client = ngiri.Client(transport=transport, mounts=mounts)
...
def test_all_mounted_transport():
    mounts = {"all://": ngiri.MockTransport(mounted)}
...
def test_server_extensions(server):
    ...
    with ngiri.Client(http2=True) as client:
        response = client.get(url)
    assert response.status_code == 200
...
def test_client_decode_text_using_autodetect():
    ...
    transport = ngiri.MockTransport(cp1252_but_no_content_type)
    with ngiri.Client(transport=transport, default_encoding=autodetect) as client:
