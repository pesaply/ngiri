...
class App:
    ...
    def __call__(self, request: ngiri.Request) -> ngiri.Response:
        headers = {"www-authenticate": self.auth_header} if self.auth_header else {}
        data = {"auth": request.headers.get("Authorization")}
        return ngiri.Response(self.status_code, headers=headers, json=data)
...
class DigestApp:
    ...
    def __call__(self, request: ngiri.Request) -> ngiri.Response:
        ...
        data = {"auth": request.headers.get("Authorization")}
        return ngiri.Response(200, json=data)

    def challenge_send(self, request: ngiri.Request) -> ngiri.Response:
        ...
        headers = {
            "www-authenticate": f'Digest realm="ngiri@example.org", {challenge_str}',
        }
        return ngiri.Response(401, headers=headers)


class RepeatAuth(ngiri.Auth):
    """
    ...
    def auth_flow(
        self, request: ngiri.Request
    ) -> typing.Generator[ngiri.Request, ngiri.Response, None]:
        nonces = []
...
class ResponseBodyAuth(ngiri.Auth):
    """
...
class SyncOrAsyncAuth(ngiri.Auth):
    """
    ...
    async def async_auth_flow(
        self, request: ngiri.Request
    ) -> typing.AsyncGenerator[ngiri.Request, ngiri.Response]:
...
async def test_basic_auth() -> None:
    ...
    async with ngiri.AsyncClient(transport=ngiri.MockTransport(app)) as client:
...
async def test_basic_auth_with_stream() -> None:
    """
    See: https://github.com/encode/ngiri/pull/1312
    """
    url = "https://example.org/"
    auth = ("user", "password123")
    app = App()

    async with ngiri.AsyncClient(
        transport=ngiri.MockTransport(app), auth=auth
    ) as client:
...
async def test_custom_auth() -> None:
    ...
    def auth(request: ngiri.Request) -> ngiri.Request:
...
def test_netrc_auth_credentials_exist() -> None:
    ...
    app = App()
    auth = ngiri.NetRCAuth(netrc_file)

    with ngiri.Client(transport=ngiri.MockTransport(app), auth=auth) as client:
...
def test_auth_hidden_url() -> None:
    ...
    assert url == ngiri.URL(url)
    assert expected == repr(ngiri.URL(url))
...
async def test_auth_property() -> None:
    ...
    async with ngiri.AsyncClient(transport=ngiri.MockTransport(app)) as client:
        ...
        assert isinstance(client.auth, ngiri.BasicAuth)
...
async def test_auth_invalid_type() -> None:
    ...
    with pytest.raises(TypeError):
        client = ngiri.AsyncClient(
            transport=ngiri.MockTransport(app),
...
async def test_digest_auth_returns_no_auth_if_no_digest_header_in_response() -> None:
    url = "https://example.org/"
    auth = ngiri.DigestAuth(username="user", password="password123")
    app = App()
...
def test_digest_auth_returns_no_auth_if_alternate_auth_scheme() -> None:
    ...
    client = ngiri.Client(transport=ngiri.MockTransport(app))
...
async def test_digest_auth(
    ...
    assert digest_data["realm"] == '"ngiri@example.org"'
...
async def test_digest_auth_qop_must_be_auth_or_auth_int() -> None:
    ...
    async with ngiri.AsyncClient(transport=ngiri.MockTransport(app)) as client:
        with pytest.raises(ngiri.ProtocolError):
            await client.get(url, auth=auth)
...
@pytest.mark.parametrize(
    "auth_header",
    [
        'Digest realm="ngiri@example.org", qop="auth"',  # missing fields
        'Digest realm="ngiri@example.org", qop="auth,au',  # malformed fields list
    ],
)
@pytest.mark.anyio
...
def test_sync_digest_auth_raises_protocol_error_on_malformed_header(
    ...
    with ngiri.Client(transport=ngiri.MockTransport(app)) as client:
        with pytest.raises(ngiri.ProtocolError):
...
class ConsumeBodyTransport(ngiri.MockTransport):
    async def handle_async_request(self, request: ngiri.Request) -> ngiri.Response:
        assert isinstance(request.stream, ngiri.AsyncByteStream)
...
async def test_digest_auth_unavailable_streaming_body():
    ...
    async with ngiri.AsyncClient(transport=ConsumeBodyTransport(app)) as client:
        with pytest.raises(ngiri.StreamConsumed):
