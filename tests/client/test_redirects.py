...
def redirects(request: ngiri.Request) -> ngiri.Response:
    if request.url.scheme not in ("http", "https"):
        raise ngiri.UnsupportedProtocol(f"Scheme {request.url.scheme!r} not supported.")

    if request.url.path == "/redirect_301":
        status_code = ngiri.codes.MOVED_PERMANENTLY
        content = b"<a href='https://example.org/'>here</a>"
        headers = {"location": "https://example.org/"}
        return ngiri.Response(status_code, headers=headers, content=content)

    elif request.url.path == "/redirect_302":
        status_code = ngiri.codes.FOUND
        headers = {"location": "https://example.org/"}
        return ngiri.Response(status_code, headers=headers)

    elif request.url.path == "/redirect_303":
        status_code = ngiri.codes.SEE_OTHER
        headers = {"location": "https://example.org/"}
        return ngiri.Response(status_code, headers=headers)
        ...
        return ngiri.Response(status_code, headers=raw_headers)
        ...
        params = ngiri.QueryParams(request.url.query)
        count = int(params.get("count", "0"))
        redirect_count = count - 1
        status_code = ngiri.codes.SEE_OTHER if count else ngiri.codes.OK
        if count:
    ...
    if request.url.path == "/redirect_loop":
        ...
        status_code = ngiri.codes.OK
        data = {
            "body": request.content.decode("ascii"),
            "headers": dict(request.headers),
        }
        return ngiri.Response(status_code, json=data)

    elif request.url.path == "/redirect_body":
        status_code = ngiri.codes.PERMANENT_REDIRECT
    ...
    if request.method == "HEAD":
        return ngiri.Response(200)

    return ngiri.Response(200, html="<html><body>Hello, world!</body></html>")


def test_redirect_301():
    client = ngiri.Client(transport=ngiri.MockTransport(redirects))
    response = client.post("https://example.org/redirect_301", follow_redirects=True)
    assert response.status_code == ngiri.codes.OK
...
def test_next_request():
    ...
    assert response.status_code == ngiri.codes.SEE_OTHER
...
async def test_async_next_request():
    async with ngiri.AsyncClient(transport=ngiri.MockTransport(redirects)) as client:
        request = client.build_request("POST", "https://example.org/redirect_303")
...
def test_malformed_redirect():
    # https://github.com/encode/ngiri/issues/771
    client = ngiri.Client(transport=ngiri.MockTransport(redirects))
    response = client.get(
...
def test_invalid_redirect():
    client = ngiri.Client(transport=ngiri.MockTransport(redirects))
    with pytest.raises(ngiri.RemoteProtocolError):
...
async def test_async_too_many_redirects():
    async with ngiri.AsyncClient(transport=ngiri.MockTransport(redirects)) as client:
        with pytest.raises(ngiri.TooManyRedirects):
            await client.get(
...
def test_can_stream_if_no_redirect():
    ...
    with client.stream("GET", url, follow_redirects=False) as response:
        pass
    assert response.status_code == ngiri.codes.MOVED_PERMANENTLY
    assert response.headers["location"] == "https://example.org/"


class ConsumeBodyTransport(ngiri.MockTransport):
    def handle_request(self, request: ngiri.Request) -> ngiri.Response:
        assert isinstance(request.stream, ngiri.SyncByteStream)
        list(request.stream)
        return self.handler(request)  # type: ignore[return-value]


def test_cannot_redirect_streaming_body():
    client = ngiri.Client(transport=ConsumeBodyTransport(redirects))
    url = "https://example.org/redirect_body"
...
def cookie_sessions(request: ngiri.Request) -> ngiri.Response:
    if request.url.path == "/":
        ...
        return ngiri.Response(200, content=content)
...
def test_redirect_cookie_behavior():
    client = ngiri.Client(
        transport=ngiri.MockTransport(cookie_sessions), follow_redirects=True
    )
...
def test_redirect_custom_scheme():
    client = ngiri.Client(transport=ngiri.MockTransport(redirects))
    with pytest.raises(ngiri.UnsupportedProtocol) as e:
