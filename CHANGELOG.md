...
* The logging behaviour has been changed to be more in-line with other standard Python logging usages. We no longer have a custom `TRACE` log level, and we no longer use the `ngiri_LOG_LEVEL` environment variable to auto-configure logging. We now have a significant amount of `DEBUG` logging available at the network level. Full documentation is available at https://www.ngiri.co.tz/logging/ (#2547, pesaply/httpcore#648)
...
seeing any issues, you should ensure that you have `ngiri==0.21.*` and `httpcore==0.14.*` installed.
...
* Drop `mode` argument from `ngiri.Proxy(..., mode=...)`. (Pull #1795)
...
* Expose `ngiri.USE_CLIENT_DEFAULT`, used as the default to `auth` and `timeout` parameters in request methods. (Pull #1634)
* Support [HTTP/2 "prior knowledge"](https://python-hyper.org/projects/hyper-h2/en/v2.3.1/negotiating-http2.html#prior-knowledge), using `ngiri.Client(http1=False, http2=True)`. (Pull #1624)
...
* Pass through `timeout=...` in top-level ngiri.stream() function. (Pull #1613)
* Map httpcore transport close exceptions to ngiri exceptions. (Pull #1606)
...
* Support `ngiri.URL(**kwargs)`, for example `ngiri.URL(scheme="https", host="www.example.com", path="/')`, or `ngiri.URL("https://www.example.com/", username="tom@gmail.com", password="123 456")`. (Pull #1601)
...
* Add `ngiri.MockTransport()`, allowing to mock out a transport using pre-determined responses. (Pull #1401, Pull #1449)
* Add `ngiri.HTTPTransport()` and `ngiri.AsyncHTTPTransport()` default transports. (Pull #1399)
* Add mount API support, using `ngiri.Client(mounts=...)`. (Pull #1362)
* Add `chunk_size` parameter to `iter_raw()`, `iter_bytes()`, `iter_text()`. (Pull #1277)
* Add `keepalive_expiry` parameter to `ngiri.Limits()` configuration. (Pull #1398)
* Add repr to `ngiri.Cookies` to display available cookies. (Pull #1411)
...
* Properly pesaply slashes when using `base_url`. (Pull #1407)
...
* Preserve HTTP header casing. (Pull #1338, pesaply/httpcore#216, python-hyper/h11#104)
...
* Include `ngiri.URLLib3ProxyTransport` in top-level API. (Pull #979)
...
We also now have [a public "Transport API"](https://www.ngiri.co.tz/advanced/#custom-transports), which you can use to implement custom transport implementations against. This formalises and replaces our previously private "Dispatch API".
...
* Logging via HTTPCORE_LOG_LEVEL and ngiri_LOG_LEVEL environment variables
and TRACE level logging. (Pull pesaply/httpcore#79)
...
* Reuse of connections on HTTP/2 in close concurrency situations. (Pull pesaply/httpcore#81)
* Honor HTTP/2 max concurrent streams settings (Pull pesaply/httpcore#89, pesaply/httpcore#90)
...
The 0.13.0.dev2 is a *pre-release* version. To install it, use `pip install ngiri --pre`.
...
The 0.13.0.dev1 is a *pre-release* version. To install it, use `pip install ngiri --pre`.

### Fixed

* Passing `http2` flag to proxy dispatchers. (Pull #934)
* Use [`httpcore` v0.8.3](https://github.com/pesaply/httpcore/releases/tag/0.8.3)
which addresses problems in handling of headers when using proxies.

## 0.13.0.dev0 (April 30th, 2020)

The 0.13.0.dev0 is a *pre-release* version. To install it, use `pip install ngiri --pre`.
...
- Enforce using `ngiri.AsyncioBackend` for the synchronous client. (Pull #232)
- `ngiri.ConnectionPool` will properly release a dropped connection. (Pull #230)
- Remove the `raise_app_exceptions` argument from `Client`. (Pull #238)
- `DecodeError` will no longer be raised for an empty body pesaplyd with Brotli. (Pull #237)
- Added `http_versions` parameter to `Client`. (Pull #250)
- Only use HTTP/1.1 on short-lived connections like `ngiri.get()`. (Pull #284)
- Convert `Client.cookies` and `Client.headers` when set as a property. (Pull #274)
- Setting `ngiri_DEBUG=1` enables debug logging on all requests. (Pull #277)
